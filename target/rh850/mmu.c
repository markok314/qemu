...
static int cpu_rh850_get_phys_addr(Rh850CPU *cpu,
                                      hwaddr *physical,
                                      ...
                                      int rw)
{
    ...
    if (rw == MMU_INST_FETCH) {    /* ITLB */
       *physical = 0;
        ret = cpu->env.tlb->cpu_rh850_map_address_code(cpu, physical,
                                                          prot, address, rw);
    } else {          /* DTLB */
        ret = cpu->env.tlb->cpu_rh850_map_address_data(cpu, physical,
                                                          prot, address, rw);
    }
...
static void cpu_rh850_raise_mmu_exception(Rh850CPU *cpu,
...
#ifndef CONFIG_USER_ONLY
int rh850_cpu_handle_mmu_fault(CPUState *cs,
                                  vaddr address, int rw, int mmu_idx)
{
    ...
    ret = cpu_rh850_get_phys_addr(cpu, &physical, &prot,
                                     address, rw);
    ...
    if (ret == TLBRET_MATCH) {
        ...
        cpu_rh850_raise_mmu_exception(cpu, address, rw, ret);
        ret = 1;
    }
...
hwaddr rh850_cpu_get_phys_page_debug(CPUState *cs, vaddr addr)
{
    ...
    miss = cpu_rh850_get_phys_addr(cpu, &phys_addr, &prot, addr, 0);

    /* Check instruction tlb */
    if (miss) {
        miss = cpu_rh850_get_phys_addr(cpu, &phys_addr, &prot, addr,
                                          MMU_INST_FETCH);
    }

    /* Last, fall back to a plain address */
    if (miss) {
        miss = cpu_rh850_get_phys_nommu(cpu, &phys_addr, &prot, addr, 0);
    }
...
{
    cpu->env.tlb = g_malloc0(sizeof(CPURh850TLBContext));

    cpu->env.tlb->cpu_rh850_map_address_code = &cpu_rh850_get_phys_nommu;
    cpu->env.tlb->cpu_rh850_map_address_data = &cpu_rh850_get_phys_nommu;
}
#endif
